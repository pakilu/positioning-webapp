...
namespace WebApp.Controllers
{
    public class ChipsController : Controller
    {
        ...
        // GET: Chips
        public async Task<IActionResult> Index(bool unassignedOnly = false)
        {
            IQueryable<Chip> query = _context.Chips
                .Include(c => c.SessionConfigChips)
                    .ThenInclude(scc => scc.SessionConfig)
                .OrderBy(c => c.Name);
            if (unassignedOnly)
            {
                query = query.Where(c => !c.SessionConfigChips.Any());
            }
            ViewData["UnassignedOnly"] = unassignedOnly;
